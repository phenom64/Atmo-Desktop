...
public:
    ...
    enum Shadow { Sunken = 0, Raised, Etched, ShadowCount };
    Render();
    ~Render(){}
...
protected:
    ...
    void splitShadowParts(const Shadow shadow, const int roundNess, const int size, const QPixmap &source);
    bool isCornerPart(const Part &part) const;
    bool needPart(const Part &part, const Sides &sides) const;
    QPixmap genPart(const Part &part, const QPixmap &source, const int roundNess) const;
